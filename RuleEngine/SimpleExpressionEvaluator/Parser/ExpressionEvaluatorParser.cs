...
namespace SimpleExpressionEvaluator.Parser
{
    public class ExpressionEvaluatorParser : Parser
    {
        ...
        public List<AbstractSyntaxTreeNode> BuildParseTree()
        {
            ...
            while (lookahead[lookaheadIndex].Type != TokenType.EOF)
            {
        ...
        {                                    
            if (lookahead[lookaheadIndex].Type == TokenType.EQUAL)
            {
                return Equal();
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.UNEQUAL)
            {
                return Unequal();
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.SMALLERTHEN)
            {
                return Smaller();
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.SMALLERTHENOREQUAL)
            {
                return Smallerequal();
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.GREATERTHEN)
            {
                return Greater();
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.GREATERTHENOREQUAL)
            {
                return Greaterequal();
            }            
            else if (lookahead[lookaheadIndex].Type == TokenType.OPENBRACKET)
            {
                return OpenBracket();
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.CLOSEBRACKET)
            {
                return CloseBracket();
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.PLUS)
            {
                return Plus();
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.MINUS)
            {
                return Minus();
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.MUL)
            {
                return Mulibly();
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.DIV)
            {
                return Divide();
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.MOD)
            {
                return Modulo();
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.IDENT)
            {
            ...
            else if (lookahead[lookaheadIndex].Type == TokenType.SET)
            {
                return Set(lookahead[lookaheadIndex].Text);
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.THEN)
            {
                return Then(lookahead[lookaheadIndex].Text);
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.ELSE)
            {
                return Else(lookahead[lookaheadIndex].Text);
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.IS)
            {
                return Is(lookahead[lookaheadIndex].Text);
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.LIKE)
            {
                return Like(lookahead[lookaheadIndex].Text);
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.NULL)
            {
                return Null(lookahead[lookaheadIndex].Text);
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.INTEGER)
            {
                return Integer(lookahead[lookaheadIndex].Text);
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.DOUBLE)
            {
                return Double(lookahead[lookaheadIndex].Text);
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.BOOL)
            {
                return Bool(lookahead[lookaheadIndex].Text);
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.STRING)
            {
                return String(lookahead[lookaheadIndex].Text);
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.OR)
            {
                return Or();
            }            
            else if (lookahead[lookaheadIndex].Type == TokenType.AND)
            {
                return And();
            }
            else if (lookahead[lookaheadIndex].Type == TokenType.COMMA)
            {
                return Comma();
            }
            return null;
